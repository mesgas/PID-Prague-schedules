...
def dig_safe(d: dict[str, Any] | None, keypath: list[str]) -> Any:
    """Like dig(), but returns None instead of raising if any step is missing or None."""
    current: Any = d
    for key in keypath:
...
# Based on the VehiclepositionCompound schema in https://api.golemio.cz/pid/docs/openapi/.
@define(kw_only=True)
class VehiclePosition:
    """Live GPS position of a single tracked trip."""
    ...
    @staticmethod
    def from_api(data: dict[str, Any]) -> VehiclePosition | None:
        """Create a VehiclePosition from the /v2/vehiclepositions/{id} API response.

        Returns None if the trip has no last known position yet (e.g. it hasn't started).
        """
        last_position = dig_safe(data, ["properties", "last_position"])
        if not last_position:
            return None
        coordinates = dig_safe(last_position, ["geometry", "coordinates"]) or [None, None]
        return VehiclePosition(
            longitude=coordinates[0],
            latitude=coordinates[1],
            bearing=dig_safe(last_position, ["properties", "bearing"]),
            speed=dig_safe(last_position, ["properties", "speed"]),
            delay_sec=dig_safe(last_position, ["properties", "delay", "actual"]),
            is_tracking=dig_safe(last_position, ["properties", "tracking"]),
            is_canceled=dig_safe(last_position, ["properties", "is_canceled"]),
        )
